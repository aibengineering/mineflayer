...
const { nextInteractionSequence } = require('../interaction_sequence')
...
function inject (bot, { hideErrors }) {
  ...
  if (bot.supportFeature('stateIdUsed')) {
    const listener = packet => { stateIds.set(packet.windowId, packet.stateId) }
  ...
  // Track the commanded hand and item identity, not the last main-hand event.
  // Vanilla continues use across same-item stack/durability updates.
  let activeOffHand = false
  let activeItemType
  let activeQuickBarSlot

  function activeItem () {
    return activeOffHand ? bot.inventory.slots[45] : bot.heldItem
  }

  function checkActiveItem () {
    if (!bot.usingHeldItem) return
    if (activeItem()?.type === activeItemType &&
        (activeOffHand || bot.quickBarSlot === activeQuickBarSlot)) return
    if (!eatingTask.done) eatingTask.finish()
    bot.usingHeldItem = false
  }

  bot.on('heldItemChanged', checkActiveItem)
  bot.inventory.on('updateSlot', checkActiveItem)

  bot._client.on('set_cooldown', (packet) => {
    if (!bot.usingHeldItem || activeItem()?.type !== packet.itemID) return
    if (!eatingTask.done) eatingTask.finish()
    bot.usingHeldItem = false
  })
  ...
  function activateItem (offHand = false) {
    // Repeated use packets do not switch an already active hand on the server.
    if (!bot.usingHeldItem) {
      activeOffHand = offHand && !bot.supportFeature('doesntHaveOffHandSlot')
      activeItemType = activeItem()?.type
      activeQuickBarSlot = bot.quickBarSlot
    }
    ...
    if (bot.supportFeature('useItemWithBlockPlace')) {
      ...
      bot._client.write('use_item', {
        hand: offHand ? 1 : 0,
        sequence: nextInteractionSequence(bot),
        rotation: {
  ...
  function deactivateItem () {
    ...
    if (bot.supportFeature('useItemWithOwnPacket')) {
      body.face = 0
      body.sequence = nextInteractionSequence(bot)
    }
  ...
  function observeCraftingResult (window, resultUpdates) {
    let remainingUpdates = resultUpdates
    let lastState = stateIds.get(window.id) ?? -1
    let resolveResult
    let rejectResult
    const promise = new Promise((resolve, reject) => { resolveResult = resolve; rejectResult = reject })
    const onSlot = packet => {
      if (packet.windowId !== window.id || packet.slot !== 0) return
      if (bot.supportFeature('stateIdUsed') && packet.stateId === lastState) return
      lastState = packet.stateId
      if (--remainingUpdates === 0) resolveResult()
    }
    const onEnd = () => rejectResult(new Error('Connection ended while waiting for crafting result'))
    const onClose = closed => {
      if (closed === window) rejectResult(new Error('Crafting window closed while waiting for result'))
    }
    const timer = setTimeout(() => rejectResult(new Error('Server did not update crafting result')), WINDOW_TIMEOUT)
  ...
  async function waitForWindowUpdate (window, slot) {
    if (window.type === 'minecraft:merchant') {
      const toUpdate = []
  ...
  async function clickWindow (slot, mouseButton, mode, craftingSlots, resultUpdates = 1) {
    ...
    // Slot.tryRemove first removes the stack, then sets the emptied slot.
    // Both operations notify a crafting grid; partial removals notify once.
    const emptiesInput = slot > 0 && mode === 0 && !window.selectedItem &&
      click.item && (mouseButton === 0 || click.item.count === 1)
    let changedSlots
    if (bot.supportFeature('transactionPacketExists')) {
      ...
      if (craftingSlots) {
        for (const { slot, item } of craftingSlots) {
          window.updateSlot(slot, item)
          if (!changedSlots.includes(slot)) changedSlots.push(slot)
        }
      }
    ...
    const craftingResult = !bot.supportFeature('transactionPacketExists') && (slot !== 0 || craftingSlots) &&
      ((window.type === 'minecraft:inventory' && slot >= 0 && slot <= 4) ||
       (window.type === 'minecraft:crafting' && slot >= 0 && slot <= 9))
      ? observeCraftingResult(window, craftingSlots ? resultUpdates : emptiesInput ? 2 : 1)
      : null
    try {
    // WHEN ADDING SUPPORT FOR OTHER CLICKS, MAKE SURE TO CHANGE changedSlots TO SUPPORT THEM
      ...
      if (bot.supportFeature('transactionPacketExists')) {
        ...
        if (craftingResult) await craftingResult.promise
        else await waitForWindowUpdate(window, slot)
      }
    } finally {
      craftingResult?.close()
    }
  }
  ...
  bot._client.on('window_items', (packet) => {
    ...
    // The cursor belongs to this authoritative window snapshot too.
  ...
  bot._takeCraftingResult = async (slots, resultUpdates) => {
    await clickWindow(0, 0, 0, slots, resultUpdates)
    if (bot.supportFeature('transactionPacketExists')) {
      const window = bot.currentWindow || bot.inventory
      for (const { slot, item } of slots) window.updateSlot(slot, item)
    }
  }
  bot.clickWindow = (slot, mouseButton, mode) => clickWindow(slot, mouseButton, mode)
