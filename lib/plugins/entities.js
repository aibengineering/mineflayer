...
function inject (bot) {
  ...
  bot._client.on('respawn', (packet) => {
    // Modern packets use a bit field: bit 0 keeps attributes, bit 1 keeps
    // entity data. Earlier packets use a boolean for keeping entity data.
    const keepEntityData = typeof packet.copyMetadata === 'number'
      ? (packet.copyMetadata & 0x02) !== 0
      : packet.copyMetadata === true
    if (!keepEntityData) bot.entity.metadata = []
  })
  ...
  bot._client.on('collect', (packet) => {
    ...
    // Collection listeners retain the item evidence from before this pickup.
    // Partial pickups can arrive without replacement item metadata.
    const item = collected.getDroppedItem()
    if (item && Number.isInteger(packet.pickupItemCount) && packet.pickupItemCount > 0) {
      item.count = Math.max(0, item.count - packet.pickupItemCount)
      const key = bot.registry.supportFeature('metadataIxOfItem')
      collected.metadata[key] = Item.toNotch(item.count > 0 ? item : null)
    }
  })
  ...
  bot._client.on('entity_metadata', (packet) => {
    ...
    if (bot.supportFeature('mcDataHasEntityMetadata')) {
      ...
      if (entity === bot.entity && metas.air_supply != null) {
